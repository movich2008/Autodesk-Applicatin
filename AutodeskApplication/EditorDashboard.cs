...
namespace AutodeskApplication
{
    public partial class EditorDashboard : Form
    {
        ...
        private readonly string editorTag;
        ...
        private string GetEditorTag(string email)
        {
            try
            {
        ...
        {
            // ✅ Log logout
            UserSessionHelper.LogSession(editorTag, editorEmail, userRole, "Logout");
        ...
        private void btnEditAsset_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                string query = @"SELECT AssetImagePath, AssetFilePath FROM Assets WHERE AssetID = @id";
                ...
                using (var cmd = new MySqlCommand(query, conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            imagePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, reader["AssetImagePath"].ToString());
                            modelPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, reader["AssetFilePath"].ToString());
                        }
                    }
                }
            }
